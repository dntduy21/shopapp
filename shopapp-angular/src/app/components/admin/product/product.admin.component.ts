...
export class ProductAdminComponent implements OnInit {
  ...
  // Hàm toggle trạng thái visible
  toggleVisibility(product: Product): void {
    const updatedVisibility = !product.visible;
    this.productService.updateVisibility(product.id, updatedVisibility).subscribe({
      next: () => {
        product.visible = updatedVisibility; // Cập nhật trực tiếp trên giao diện
        alert(
          `Sản phẩm "${product.name}" đã được ${updatedVisibility ? 'hiện' : 'ẩn'
          } thành công!`
        );
      },
      error: (err) => {
        console.error('Có lỗi xảy ra khi cập nhật trạng thái:', err);
        alert('Không thể thay đổi trạng thái hiển thị của sản phẩm.');
      },
    });
  }
}